...
(Tropp 2011) that we saw in the previous section (\Cref{thm:matbernstein}).
\begin{theorem}\label{thm:matbernsteinagain}
...
We say that a function $f : \calS \to \R$, where $\calS \subseteq \symsetn$,
  ...
  $f(\AA) \preceq f(\BB)$.
%
\begin{lemma}\label{lem:trpmonoinc}
  Let $T \subseteq \R$.
...
The following is a more general version of Lemma~\ref{lem:trpmonoinc}
\begin{lemma}
  Let $T \subset \R$.
