...
Then $\xx^\trp \AA\xx= \xx^\trp \VV
\LLambda\VV^\trp\xx= (\VV^\trp\xx)^\trp  \LLambda(\VV^\trp\xx) = \sum_{j = 1}^i
...
\[
 ...
 \sum_{j \leq i} a_j  f(\xx_j)
   + \ip{ a_j\grad
     f(\xx_{j}) ,\vv - \xx_j}
 \]
Thus $A_iL_i-A_{i+1}L_{i+1} = m_{i}(\vv_i) - m_{i+1}(\vv_{i+1})$.
